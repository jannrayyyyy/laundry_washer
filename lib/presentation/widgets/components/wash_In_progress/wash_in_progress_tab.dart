...
class WashInProgressTab extends StatefulWidget {
  final TextEditingController searchProductController;
  const WashInProgressTab({
    super.key,
    required this.searchProductController,
  });

  @override
  State<WashInProgressTab> createState() => _WashInProgressTabState();
}

class _WashInProgressTabState extends State<WashInProgressTab> {
  int loads = 0;
  int kg = 0;
  int qty = 0;
      ...
      dataTable: BlocBuilder<BookingCubit, BookingsState>(
        builder: (context, state) {
          if (state is BookingsLoaded) {
            final bookings =
                state.bookings.where((element) => element.bookingStatus == 3);
                    ...
                    builder: (ctx) {
                      return CustomDialog(
                        ...
                        onTap: () {
                          context
                              .read<BookingCubit>()
                              .updateBookingStatus(bookingUids, 4);
                          for (var element in bookings) {
                            if (element.isSelected!) {
                              element.isSelected = false;
                              setState(() {});
